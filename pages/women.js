...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    if (item.title in women) {
      if (!women[item.title].color.includes(item.color) && item.availableQty > 0) {
        women[item.title].color.push(item.color)
      }

      if (!women[item.title].size.includes(item.size) && item.availableQty > 0) {
        women[item.title].size.push(item.size)
      }
    }

    else {
      women[item.title] = JSON.parse(JSON.stringify(item))
      if (item.availableQty > 0) {
        women[item.title].color = [item.color]
        women[item.title].size = [item.size]
      }
    }
  }

  return {
    props: { "products": JSON.parse(JSON.stringify(women)) }, // will be passed to the page component as props
  }
}
